...
{
    FILE* fp;
    char* php = malloc(sizeof(char)*(strlen(path)+10));
    char* html = malloc(sizeof(char)*(strlen(path)+11));
    char* index = "index";
    char* phpExt = ".php\0";
    char* htmlExt = ".html\0";
    int i = 0;
...
{
    BYTE* p;
    p = malloc(sizeof(BYTE));
    if(p == NULL) {
        return false;
    }
    int i = 0;
    do {
        p = realloc(p, sizeof(BYTE)*(i+1));
        p[i] = fgetc(file);
        i++;
        if(p == NULL) {
            return false;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    int size = 0;
    int place = 0;
    method = (char*) malloc(sizeof(char)*strlen(line));
    target = (char*) malloc(sizeof(char)*strlen(line));
    http = (char*) malloc(sizeof(char)*strlen(line));
    query = (char*) malloc(sizeof(char)*strlen(line));
    p = method;
    do {
        if(line[i] == ' ') {
            size = 0;
    ...
    bool allIsGood = true;
    if(strncmp(method, "GET\0", sizeof(char)*3) != 0) {
        error(401);
        allIsGood = false;
    } else if(strncmp(target, "/",  sizeof(char)) != 0) {
        error(501);
        allIsGood = false;
    } else if(strstr(target, "\"")) {
        error(400);
        allIsGood = false;
    } else if(strncmp(http, "HTTP/1.1\0",  sizeof(char)*9) != 0) {
        error(505);
        allIsGood = false;
    }
